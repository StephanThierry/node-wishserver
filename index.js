...
/*
//Implementing this pattern can be nessesary if unexpected events make the service fail
...
app.post("/login",function(req,res){
  ...
  if (person_password==null) console.log('person_password : null');
...
app.get("/who",function(req,res){
  ...
  if (person_id==null) console.log('req.session.person_id : null');
...
function outputHandler(err, res, rows, mode, req){
  ...
  if (!err){
    if (mode=="select" || mode=="delete") res.send(rows);
    if (mode=="insert") res.send({'insertId':rows.insertId});
    if (mode=="login") {
        if (rows[0]!=undefined) {
         ...
         req.session.save(function(err) {
           // session saved, we assume it went ok
         });
       } else {
